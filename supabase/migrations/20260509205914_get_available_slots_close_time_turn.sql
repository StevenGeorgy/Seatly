-- Fix: get_available_slots() was offering start times whose 90-min turn
-- ran past the shift's close. The slot loop checked `v_slot_inc` (15 min,
-- the slot increment) instead of `v_turn_mins`, so a Saturday shift that
-- ends at 23:00 with a 90-min turn would still emit 22:45 as bookable
-- — that booking actually runs to 00:15 the next day. Switching the
-- bound to `v_turn_mins` makes the last bookable start time
-- `(close - turn)` (e.g. 21:30 for 23:00 close + 90-min turn).
--
-- Body otherwise byte-identical to 20260508000700_get_available_slots_timezone.sql.
...
BEGIN
  ...
  LOOP
    ...
    -- BUGFIX: was `v_slot_min + v_slot_inc <= v_end_min`, which let
    -- a slot's start hug the close time even though the booking
    -- itself runs `v_turn_mins` past it.

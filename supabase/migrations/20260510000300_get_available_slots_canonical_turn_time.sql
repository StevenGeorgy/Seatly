-- Make get_available_slots use the canonical restaurant_turn_time_minutes()
-- helper, which prefers restaurants.settings_json.turnTimeMinutes (the value
-- the dashboard owner sets) over the per-shift column. This eliminates a
-- silent drift where the dashboard said 90 but a shift's turn_time_minutes
-- column was a stale 60 (caught at Georgy Inc lunch on 2026-05-10).
--
-- All other booking RPCs (book_reservation, modify_reservation_slot,
-- create_staff_reservation) already use this helper. This migration brings
-- get_available_slots into parity.

CREATE OR REPLACE FUNCTION public.get_available_slots(
  p_restaurant_id uuid, p_date text, p_party_size integer
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT timezone, hours_json INTO v_restaurant_row
  ...
  v_hours_json := CASE
    ...
    THEN v_restaurant_row.hours_json ELSE NULL END;
  ...
  FROM public."tables" WHERE restaurant_id = p_restaurant_id AND is_active = true;
    ...
    IF v_special IS NOT NULL THEN
      ...
      ELSE
        SELECT * INTO v_pair FROM public._availability_read_hours_pair(jsonb_build_object(
          'open', v_special -> 'from', 'close', v_special -> 'to', 'closed', false
        ));
  ...
  LOOP
    ...
    IF v_request_date > v_max_booking_date THEN CONTINUE; END IF;

    IF v_shift.blackout_dates IS NOT NULL
       AND v_request_date = ANY(v_shift.blackout_dates) THEN CONTINUE; END IF;

    v_slot_inc := COALESCE(v_shift.slot_duration_minutes, 15);
    -- Use canonical helper so dashboard turn time (settings_json.turnTimeMinutes)
    -- always wins over a stale shift.turn_time_minutes column.
    v_turn_mins := COALESCE(public.restaurant_turn_time_minutes(p_restaurant_id, v_shift.id), 90);
    ...
    END IF;
    IF v_end_min <= v_slot_min THEN CONTINUE; END IF;
    ...
    WHILE v_slot_min + v_turn_mins <= v_end_min LOOP
      ...
      IF v_max_covers IS NULL OR v_total_covers <= v_max_covers THEN
        ...
        ELSE
          ...
          IF jsonb_array_length(v_slots_arr) >= 48 THEN EXIT; END IF;
        END IF;
      END IF;

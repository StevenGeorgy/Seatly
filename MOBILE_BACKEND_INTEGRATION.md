...
| `409 unavailable_reason: "past_shift_close"` | `reserved_at + turn` would run past the shift's `end_time` | "This time is past the shift's close. Pick an earlier slot." Refetch `get_available_slots` to confirm the offered slots are still valid. |
...
- **Identifier requirement (HARD):** every row must have at least one of `user_profile_id`, `guest_email` (non-empty), or `guest_phone` (digits-only non-empty). Enforced by `reservations_must_have_identifier` CHECK + ALL THREE writers raise P0007 early: `book_reservation`, `create_staff_reservation`, `modify_reservation_slot` (modify reads the existing row's identifiers — defensive). `guest_id` alone is NOT enough.
...
- **Staff-flow opaque error: FIXED 2026-05-09.** `create_staff_reservation` and `modify_reservation_slot` now raise friendly `P0007 / missing_identifier` (parity with `book_reservation`). Edge function `modify-reservation` maps it to a 400 with `unavailable_reason: 'missing_identifier'`. Migration `20260509220000_reservation_writers_identifier_parity.sql`.
- **`get_available_slots` close-time loop: FIXED 2026-05-09.** Slot generation no longer offers start times whose `+ turn_minutes` runs past close. Last bookable start = `close − turn`. Migration `20260509205914_get_available_slots_close_time_turn.sql`. Mobile clients caching slots from before this fix may still show stale 22:45-style starts; one cache-bust + refetch resolves it.
- **Booking-RPC close-time guard: FIXED 2026-05-09.** Beyond the slot-generator fix, `book_reservation` / `modify_reservation_slot` / `create_staff_reservation` themselves now reject post-close starts (`P0008 / past_shift_close` mapped to 409 in the public + modify edge functions; cenaiva-chat surfaces a similar message). Closes the bypass where any client could POST an arbitrary `date_time` and the RPC trusted it. Migration `20260509230000_booking_close_time_and_confirmation_return.sql`. Same-day shifts only — overnight shifts (start > end) are still un-enforced; the slot generator doesn't support them either.
- **Confirmation code returned by `book_reservation` is the persisted DB value: FIXED 2026-05-09.** Before this fix, the BEFORE-INSERT trigger `reservations_confirmation_code` generated a fresh 8-hex code that overrode whatever `p_confirmation_code` was supplied, but the RPC still returned the input value. SMS / email / API response had a code the DB didn't have, breaking customer-side modify/cancel by confirmation_code. Now the RPC's `RETURNING id, confirmation_code INTO …` captures the trigger-set value; mobile clients reading `bookingRow.confirmation_code` get the correct code with no client change required.
- **Voice modify/cancel bypassed all safety guards: FIXED 2026-05-09 (cenaiva-orchestrate v169).** Voice (`cenaiva-orchestrate`) used to write `reservations.update({reserved_at, shift_id, party_size})` directly for modify and `update({status:"cancelled"})` for cancel — skipping advisory lock, diner-overlap, cover-cap, close-time guard (P0008), and table reassignment. Voice modify now routes through `modify_reservation_slot` RPC (with friendly spoken-text mapping for P0001/P0002/P0004/P0005/P0006/23P01/P0008). Voice cancel does the status flip + `release_reservation_tables` RPC. Mobile cherry-picks of cenaiva-orchestrate should pull v169 or later. Important for mobile: nothing in the mobile client needs to change — the orchestrator's response shape (the SSE `assistant_payload`) is unchanged; only the DB-level behavior tightened.
- **Voice search now filters by `business_type` AND accepts any city: FIXED 2026-05-09 (cenaiva-orchestrate v170).** The `search_restaurants` tool's JSON schema gained a `business_type` parameter (cafe / bar / brewery / bistro / coffee shop / bakery / pub / lounge), the system prompt explicitly lists smaller Canadian cities as valid `city` values (Guelph, Hamilton, Kitchener, Kingston, Saskatoon, Halifax, Quebec City, etc.), and the SQL query SELECTs + ILIKE-filters both `business_type` and `cuisine_type`. Mobile cherry-picks should pull v170 or later. The orchestrator's response shape is unchanged. Three small frontend changes shipped alongside in `apps/web`: a non-empty hint card on `RestaurantRail` when search returns zero matches, a defensive `stopListening()` + 200ms gap before `startListening()` after the wake greeting (so the mic auto-opens reliably), and 9 more phrases pre-cached in the IDB TTS store.
- **`useAvailabilityRealtimeInvalidate` multiplexed 2026-05-09.** Multiple components on the same page can subscribe to the same `restaurantId`; the hook holds a module-level registry that shares one socket. Mobile mirror: keep a similar registry if you mount the booking widget more than once on the same screen.
- **Reservation date filters must use restaurant timezone, not UTC.** The web dashboard had a Sun-view-shows-Sat-late-bookings bug because PostgREST interpreted `T00:00:00` as UTC. Fixed with `localDayBoundsUtcIso(date, tz)` (see `apps/web/src/lib/utils/time.ts`). Mobile staff dashboards (if any) should compute the same UTC bounds before querying `reservations` by `reserved_at`.
...
- **Live-vs-tracked migration drift CLOSED 2026-05-09.** Three previously-deployed migrations have been backfilled into `supabase/migrations/`: `20260509081135_reservations_require_identifier.sql`, `20260509201816_add_owner_registration_columns.sql`, `20260509205914_get_available_slots_close_time_turn.sql`. A fresh `supabase db reset` now produces a schema identical to prod. There may still be older drift (auth/storage system migrations); treat the production schema as authoritative when uncertain.
- **`restaurants` gained owner-registration columns** (2026-05-09): `owner_user_id`, `stripe_subscription_id`, `stripe_payment_method_id`, `billing_card_brand/last4/exp_month/exp_year`, `trial_ends_at`. All nullable, mobile customer flows can ignore them. Used by the owner-signup web flow.

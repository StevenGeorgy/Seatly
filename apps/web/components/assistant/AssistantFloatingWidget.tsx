...
export function AssistantFloatingWidget() {
  ...
  return (
    <div className="fixed bottom-md right-md z-50 flex flex-col items-end gap-sm">
      {open ? (
        <div
          className="w-full max-w-sm overflow-hidden app-card-elevated"
          aria-hidden={!open}
        >
          <div className="flex items-center justify-between gap-md border-b border-card-border px-md py-sm">
            <p className="text-sm font-semibold text-text-on-dark">AI Assistant</p>
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="rounded-md border border-card-border px-sm py-xs text-sm text-text-muted-on-dark transition-colors duration-200 hover:border-gold/40 hover:text-text-on-dark"
            >
              Close
            </button>
          </div>
